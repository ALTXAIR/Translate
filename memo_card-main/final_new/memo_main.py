...
def set_card():
    ''' задає, як виглядає вікно карточки'''
...
def sleep_card():
    ''' карточка ховається на деякий час, вказанний в таймері'''
    win_card.hide()
...
def click_OK():
    ''' перевіряє запитаня чи завантажує нове питання '''
...
def back_to_menu():
    ''' повернення з тесту до вікно редактора '''
    win_card.hide()
    win_main.showNormal()
...
def edit_question(index):
    ...
    #  index - це об'єкт класу QModelIndex, див. потрібні методи ниже
...
# а не при підключенні як модуль. Дітям це зовсім не потрібне. Жах
testlist()
set_card()
set_main()
connects()
